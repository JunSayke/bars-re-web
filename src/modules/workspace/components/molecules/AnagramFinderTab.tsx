...
interface AnagramFinderTabProps {
...
export function AnagramFinderTab({ onSelectWord }: AnagramFinderTabProps) {
  ...
  const entries = result?.anagrams ?? []
  const hasNoResults = !isLoading && !isError && submittedTerm && entries.length === 0

  return (
    <div className="flex flex-col gap-4 h-full">
      <form onSubmit={handleSubmit} className="flex flex-col gap-1.5">
        <label htmlFor="anagram-query" className="text-xs font-medium text-muted-foreground">
          Query
        </label>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {hasNoResults && (
          <p className="text-xs text-muted-foreground italic">
            No anagrams found for &ldquo;{submittedTerm}&rdquo;.
          </p>
        )}
