...
- **WHEN** the user submits a new link while a `beat_links` row already exists for the session (regardless of the previous provider)
...
- **WHEN** the editor mounts for a session that has a `beat_links` row with `provider = "spotify"`
...
- **WHEN** the editor mounts for a session that has a `beat_links` row with `provider = "youtube"`
...
- **WHEN** the editor mounts for a session that has a `beat_links` row with `provider = "soundcloud"`
