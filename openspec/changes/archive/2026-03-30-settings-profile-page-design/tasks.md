...
- [ ] 4.2 Verify avatar preview updates live on the Profile Settings page by running `pnpm dev` and editing the Avatar URL field
- [ ] 4.3 Verify the Account Settings page renders correctly at `/settings/account` in dev mode
- [ ] 4.4 Verify clicking the gear icon in both `WorkspacesTopNav` and `EditorTopNav` navigates to `/settings/profile`
